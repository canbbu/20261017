import { MapPin, Phone } from "@phosphor-icons/react/dist/ssr";
import { wedding } from "@/content/wedding";
import { Accordion } from "@/components/interactive/Accordion";
import { CopyButton } from "@/components/interactive/CopyButton";
import { Reveal } from "@/components/interactive/Reveal";
import { isPendingValue } from "@/lib/content";
import { getSafeMapLinks } from "@/lib/maps";
import { hasPhone } from "@/lib/content";
import { toTelHref } from "@/lib/phone";

const TRANSPORT_LABELS = {
  subway: "지하철",
  bus: "버스",
  car: "자가용",
  parking: "주차",
} as const;

export function Venue() {
  const { event } = wedding;
  const mapLinks = getSafeMapLinks(event.mapLinks);
  const venueTel = hasPhone(event.phone) ? toTelHref(event.phone) : null;
  const transportItems = (
    Object.keys(TRANSPORT_LABELS) as Array<keyof typeof TRANSPORT_LABELS>
  )
    .map((key) => ({ key, label: TRANSPORT_LABELS[key], value: event.transportation[key] }))
    .filter((item) => item.value.trim().length > 0);

  return (
    <section className="section bg-canvas" aria-labelledby="venue-title">
      <Reveal className="page-shell mx-auto">
        <h2 id="venue-title" className="section-title">
          오시는 길
        </h2>
        <div className="info-card">
          <p className="font-serif text-[1.35rem]">{event.venueName}</p>
          {isPendingValue(event.hall) ? null : <p className="mt-1 text-ink-muted">{event.hall}</p>}
          <p className="mt-2 text-[0.9375rem] text-ink-muted">{event.address}</p>

        <div className="mt-5 grid grid-cols-2 gap-2">
          {isPendingValue(event.address) ? (
            <p className="caption text-ink-muted">주소는 아직 입력되지 않았습니다.</p>
          ) : (
            <CopyButton value={event.address} label="주소 복사" />
          )}
          {venueTel ? (
            <a
              href={venueTel}
              className="control inline-flex items-center justify-center gap-2 border border-line bg-surface text-[0.9375rem]"
              aria-label={`${event.venueName}에 전화하기`}
            >
              <Phone size={18} weight="regular" aria-hidden="true" />
              전화하기
            </a>
          ) : null}
        </div>

        {mapLinks.length > 0 ? (
          <div className="mt-2 grid grid-cols-2 gap-2">
            {mapLinks.map((link) => (
              <a
                key={link.id}
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                className="control inline-flex items-center justify-center gap-2 border border-line bg-surface-muted text-[0.9375rem] text-ink"
                aria-label={`${link.label}에서 ${event.venueName} 길찾기 열기`}
              >
                <MapPin size={18} weight="regular" aria-hidden="true" />
                {link.label}
              </a>
            ))}
          </div>
        ) : (
          <p className="caption mt-4 text-ink-muted">
            지도 링크는 아직 입력되지 않았습니다. 공식 지도 이미지는 넣지 않았습니다.
          </p>
        )}

        {transportItems.length > 0 ? (
          <div className="mt-5 border-t border-line pt-2">
            {transportItems.map((item) => (
              <Accordion key={item.key} title={item.label}>
                <p className="whitespace-pre-wrap">{item.value}</p>
              </Accordion>
            ))}
          </div>
        ) : (
          <p className="caption mt-5 border-t border-line pt-4 text-ink-muted">교통 안내는 아직 입력되지 않았습니다.</p>
        )}
        </div>
      </Reveal>
    </section>
  );
}
